...
namespace HeadlessTerrariaClient.Utility
{
    ...
    public static class ClientExtensions
    {
        ...
        {
            client.LocalPlayer.position = position;
            ...
            lock (client.WriteBuffer)
            {
                ...
                switch (messageType)
                {
                    ...
                    case MessageID.PlayerControls:
                    {
                        ...
                        writer.Write((byte)0);
                        // Pulley
                        BitsByte miscData1 = (byte)0;
                        // Include velocity payload to keep packet layout valid.
                        miscData1[2] = true;
                        miscData1[4] = true;
                        writer.Write((byte)miscData1);
                        // Misc
                        writer.Write((byte)0);
                        // SleepingInfo
                        writer.Write((byte)0);
                        // Selected Item
                        writer.Write((byte)Math.Clamp(client.LocalPlayer.selectedItem, 0, 58));
