...
namespace HeadlessTerrariaClient.Terraria
{
	...
	public class NetworkText
	{
		...
		/// <summary>
		/// Returns a diagnostic string including substitutions for localization keys.
		/// Useful when localization is unavailable and ToString() only returns key names.
		/// </summary>
			...
			if (_mode == Mode.LocalizationKey)
			{
				...
				if (_substitutions != null && _substitutions.Length > 0)
				{
					...
					for (int i = 0; i < _substitutions.Length; i++)
					{
						...
						builder.Append(_substitutions[i].ToString());
					}
				}
