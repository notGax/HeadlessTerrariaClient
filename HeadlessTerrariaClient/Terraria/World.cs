...
namespace HeadlessTerrariaClient.Terraria
{
	public class World
    {
        ...
        public void DecompressTileSection(byte[] buffer, int bufferStart, int bufferLength, bool loadTileSections)
		{
			...
			byte[] payload = new byte[bufferLength];
			Buffer.BlockCopy(buffer, bufferStart, payload, 0, bufferLength);

			MemoryStream tileSectionStream;
			byte compressionType = payload[0];

			// Vanilla commonly uses:
			// 0 = raw section bytes (no compression), 1 = deflate stream.
			// If the first byte is not a known compression flag, fall back to "no flag" raw parsing.
			if (compressionType == 0)
			{
				tileSectionStream = new MemoryStream(payload, 1, payload.Length - 1, writable: false);
			}
			else if (compressionType == 1)
			{
				...
				try
				{
					using MemoryStream compressedStream = new MemoryStream(payload, 1, payload.Length - 1, writable: false);
					using DeflateStream deflateStream = new DeflateStream(compressedStream, CompressionMode.Decompress, leaveOpen: false);
					deflateStream.CopyTo(decompressedSectionStream);
					decompressedSectionStream.Position = 0L;
					tileSectionStream = decompressedSectionStream;
				}
				catch
				{
					// Fallback: try reading as raw/no-compression payload in case server used different flag semantics.
					tileSectionStream = new MemoryStream(payload, writable: false);
				}
			}
			else
			{
